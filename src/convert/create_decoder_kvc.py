...
def parse_arguments():
    ...
    parser.add_argument("-n", '--name-model', metavar = "str", type = str, default = 'SmolVLM-256M-Instruct',
...
def main():    
    ...
    modified_model_path = args.output_dir

    model_id = "HuggingFaceTB/SmolVLM-256M-Instruct"
    config = AutoConfig.from_pretrained(model_id)

    num_layers = config.text_config.num_hidden_layers
    num_heads = config.text_config.num_key_value_heads
    hidden_cache_size = config.text_config.head_dim


    # Model parameters
    # if "llama3-8b" in args.name_model:
    #     num_layers = 32
    #     num_heads = 8
    ...
    model = onnx.load(original_model_path, load_external_data=False)
    ...
    old_total_sequence_names = ["/model/Unsqueeze_1_output_0", "/model/Unsqueeze_2_output_0", "/model/Unsqueeze_7_output_0", "/model/Unsqueeze_8_output_0"]
