...
# from src.session.tinygrad.module import TinyModule
...
class SmolVLMWithExpertModelTinyOnnx(TinyOnnxModule):
    ...
    def get_vlme_module(
        ...
        self.vlm_module_prefill = TinyOnnxModule(
            model_path=model_path_prefill,
            hf_repo=self.hf_repo,
            hf_filename="smolvlm_expert_prefill.onnx",
        )
        self.vlm_module_decode = TinyOnnxModule(
    ...
    def get_text_encoder_module(
        self, model_path: Optional[Path] = None, provider="CPU"
    ) -> TinyOnnxModule:
        module = TinyOnnxModule(
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            outputs_embeds, *past_key_values = self.vlm_module_decode(**input_feed)
        else:

            outputs_embeds, *past_key_values = self.vlm_module_prefill(**input_feed)
